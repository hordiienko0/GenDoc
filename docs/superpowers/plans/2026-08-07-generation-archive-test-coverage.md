...
| ~~C1 — шум у `unfilledTags`~~ | **спростовано** | Task 9 (зелена гарантія) |
| ~~C2 — `{{дата_аркуша}}` у формі ручних тегів~~ | **знято з обсягу** | — |
| C3 — «не документ Word» на будь-яку причину | Task 16 | Task 16 |
| C4 — блок у таблиці без контексту | Task 17 | Task 17 |

**Task 15 скасовано.** C1 виявився хибним діагнозом: кожен успішний шлях підстановки
переписує текст клітинки, тож `CollectResidualUnfilledTags` фізично не може побачити
заповнений тег, а `Distinct()` знімає питання дублікатів. Разом з C1 замовник зняв і
C2. Task 9 замість червоного тесту дістав зелену гарантію
`UnfilledTags_ExcludeTagsThatWereSubstituted`.
